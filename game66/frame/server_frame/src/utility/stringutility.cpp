...
/** 在字符串中查找子符串，不区分大小写 */
std::string::size_type CStringUtility::FindNoCase(const std::string & strSrc, const std::string & strSub, size_t dwFrom)
{
	std::string::const_iterator it  = std::search(strSrc.begin()+dwFrom, strSrc.end(), strSub.begin(),strSub.end(), CmpNoCase);
...
/** 在字符串中查找子符串，不区分大小写 */
std::string::size_type CStringUtility::RFindNoCase(const std::string & strSrc, const std::string & strSub, size_t dwFrom)
{
	std::string::const_iterator it = std::find_end(strSrc.begin(), strSrc.begin()+dwFrom, strSub.begin(),strSub.end(), CmpNoCase);
...
bool CStringUtility::IsOnlyComposedOfGB2312Cha(const std::string& strSrc)
{
	...
	for (size_t i = 0; i+1 < strSrc.size(); i=i+2)
	{
		...
		szTmp[0] = strSrc[i]; szTmp[1] = strSrc[i+1];
...
bool CStringUtility::IsDBCS(const unsigned char szStr[2])
{
	return ((szStr[0]>0xa0 && szStr[1]>0xa0) 								//HZ_GB
        ...
        || (((szStr[0] == 0xa1)||(szStr[0]==0xa3)) && szStr[1]>0xa0)						//HZ_TSIGN
...
bool CStringUtility::CheckGbKString(const std::string& strSrc, bool bContainsNprint)
{
	...
	for (size_t i = 0; i < nLen  ; i++)
	{
		if (strSrc[i] < 0)
		{
			...
			cTempChar[0] = (unsigned char)strSrc[i];  cTempChar[1] = (unsigned char)strSrc[i+1];  
			if (IsDBCS(cTempChar))
			{
				i++;
				continue;
...
std::string CStringUtility::DecodeUrlStr(const std::string &strSrc)
{
	...
	for (size_t i=0; i<strSrc.size(); i++)
	{
		if (strSrc[i] == '%' && (i+2) < strSrc.length() && IsHexChar(strSrc[i+1]) && IsHexChar(strSrc[i+2]))
		{
			char cTempChar[3]; cTempChar[2] = 0;
			cTempChar[0] = strSrc[i+1];  cTempChar[1] = strSrc[i+2];  
...
bool CStringUtility::GetSubStrByPrefixSuffix(const std::string &strSrc, std::string &strSub,const  std::string& strPrefix, const std::string& strSuffix, size_t iFromIndex, bool bNoCase)
{
...
ssize_t CStringUtility::BinBufToHexBuf(char *pHexBuf, size_t iHexBufSize, const unsigned char *pBinBuf, size_t iBinBufSize)
{
...
ssize_t CStringUtility::BinBufToHexStr(char *pszHexStrBuf, size_t iHexStrBufSize, const unsigned char *pBinBuf, size_t iBinBufSize)
{
...
ssize_t CStringUtility::BinBufFromHexBuf(unsigned char *pBinBuf, size_t iBinBufSize, const char *pHexBuf, size_t iHexBufSize)
{
	if (iHexBufSize > (iBinBufSize << 1))
	{
		// dst buffer space is not enough
        ...
        // value
        	if (i < (iLine - 1) || (iDataSize & 0xf) == 0)
        	{
            		for (size_t j = 0; j < 16; ++j)
            		{
                		...
                		iPos += snprintf(pszOutBuf + iPos, iOutBufSize - 1 - iPos, "%c", isprint(pBegin[j]) > 0 ? pBegin[j] : '.');
...
std::string CStringUtility::FormatToString(const char* fmt,...)
{
	...
	va_list ap;
	va_start(ap, fmt);
	vsnprintf(szTmp,sizeof(szTmp),fmt,ap);
	va_end(ap);
