...
{
	char temp[64];
	dSprintf(temp,sizeof(temp), "%lld", *(long long*)lua_topointer(L, 1));
...
{
	char temp[64];
	dSprintf(temp,sizeof(temp), "%llu", *(unsigned long long*)lua_topointer(L, 1));
	lua_pushstring(L, temp);
	return 1;
}
...
void lua_tinker::print_error(lua_State *L, const char* fmt, ...)
{
	...
	va_list args;
	va_start(args, fmt);
	dVsprintf(text,sizeof(text), fmt, args);
	va_end(args);
