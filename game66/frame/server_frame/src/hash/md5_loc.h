...
//We are on Linux/x86 and so is little endian.
/*
 * little endian
 */
#define SWAP(n)	(n)
 ...
 */
#define OP1(a, b, c, d, b_p, c_p, s, T) do {memcpy(c_p, b_p, sizeof(uint32_t));*c_p = SWAP(*c_p);a += FF (b, c, d) + *c_p + T;a = CYCLIC (a, s);a += b; b_p = (char *)b_p + sizeof(uint32_t);c_p++;} while (0)
 ...
 */
#define OP234(FUNC, a, b, c, d, k, s, T) do {a += FUNC (b, c, d) + k + T;a = CYCLIC (a, s);a += b;} while (0)
