        ...
        uint32_t buf_len)
{
  ...
  words_n = buf_len / sizeof(uint32_t);
  end_p = (char *)buf_p + words_n * sizeof(uint32_t);
...
static	void	md5_get_result(const md5_t *md5_p, void *result)
{
  ...
  memcpy(res_p, &hold, sizeof(uint32_t));
  res_p = (char *)res_p + sizeof(uint32_t);
...
void	md5_finish(md5_t *md5_p, void *signature)
{
   ...
   */
  pad = MD5_BLOCK_SIZE - (sizeof(uint32_t) * 2) - bytes;
  if (pad <= 0) {
    pad += MD5_BLOCK_SIZE;
  }
  ...
  memcpy(md5_p->md_buffer + bytes, &hold, sizeof(uint32_t));
