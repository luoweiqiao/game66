 ...
 */
#ifndef MULTISTEP_HASHTABLE_
#define MULTISTEP_HASHTABLE_
...
public:
    ...
    CHashTable(uint32_t uiMaxElementNumber, uint8_t* pData,
            ...
            const Comparator& oComparator = Comparator()) /*throw (const char*)*/ :
                ...
                m_iterateIndex(0), m_Comparator(oComparator)
    {
        ...
        if (bIsLRU)
        {
            m_puiCurrentLRUCounter = (uint32_t *) (pData + CHashTable<KEY,
                    VALUE, true>::GetNeededDataSize(m_uiMaxElementNumber, m_u32HashTime)
                    - sizeof(uint32_t));
        }
        else
        {
    ...
    static size_t GetNeededDataSize(uint32_t uiMaxElementNumber, uint32_t u32HashTime = 50)
    {
        size_t size = sizeof(uint32_t) + sizeof(uint32_t) + u32HashTime
                * sizeof(uint32_t) + GetRoundedUpNumber(uiMaxElementNumber, u32HashTime)
                * sizeof(ITEMTYPE);
        if (bIsLRU)
        {
