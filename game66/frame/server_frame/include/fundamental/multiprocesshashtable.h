/*
 * 文件名：multiprocesshashtable.h
 * 作者: toney
 * 日期: 2011-11-18

 * 描述：适用于多进程的共享内存HashTable实现。
 */
#ifndef CMULTIPROCESSHASHTABLE_H_
#define CMULTIPROCESSHASHTABLE_H_
...
public:
     ...
     *     中会自动向上往50的整数倍取整。
     */
    bool InitShm(int iShmKey, uint32_t uiMaxElementNumber,uint32_t u32HashTime = 50) /*throw (const char*)*/
    {
        m_pShm = new CShm(iShmKey,
                CHashTable<KEY, VALUE, bIsLRU>::GetNeededDataSize(uiMaxElementNumber, u32HashTime));
        if (m_pShm->Get() == NULL)
        {
            return false;
        }
...
#endif /* CMULTIPROCESSHASHTABLE_H_ */
