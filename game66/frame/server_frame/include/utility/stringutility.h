...
public:
	...
	*/
	...
	static void ParserNumberString( std::vector<TYPE> & RefNumVec, std::string & RefString, char chSeg, bool bFilterZero = false )
	{
		if( RefString.empty() )
		{
			return ;
	...
	/**
			...
			@return 匹配的位置
		*/
	static std::string::size_type FindNoCase(const std::string & strSrc, const std::string & strSub, size_t dwFrom = 0);
	static std::string::size_type RFindNoCase(const std::string & strSrc, const std::string & strSub, size_t dwFrom = std::string::npos);
	...
	*/
	...
	/**
		得到前缀和后缀之间的字符串
			...
			处理后的串
	*/
	static bool GetSubStrByPrefixSuffix(const std::string &strSrc, std::string &strSub,const  std::string& strPrefix, const std::string& strSuffix, size_t iFromIndex = 0, bool bNoCase = true);
	...
	*/
	...
	static ssize_t BinBufToHexBuf(char *pHexBuf, size_t iHexBufSize, const unsigned char *pBinBuf, size_t iBinBufSize);
	static ssize_t BinBufToHexStr(char *pszHexStrBuf, size_t iHexStrBufSize, const unsigned char *pBinBuf, size_t iBinBufSize);

	static std::string BinBufToHexString(const uint8_t *pBuf, size_t uiLen);

	static ssize_t BinBufFromHexBuf(uint8_t *pBinBuf, size_t iBinBufSize, const char *pHexBuf, size_t iHexBufSize);
