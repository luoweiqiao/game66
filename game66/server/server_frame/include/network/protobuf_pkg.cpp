...
bool  SendProtobufMsg(NetworkObject* pNetObj,const google::protobuf::Message* msg,uint16 msg_type,uint32 uin)
{
	if(pNetObj == NULL)
	{
		//LOG_DEBUG("3 Socket Send Msg To Client-cmd:%d--uin:%d", msg_type, uin);
    ...
    pkt.header.uin = uin;
	//LOG_DEBUG("1 Socket Send Msg To Client-cmd:%d--len:%d",pkt.header.cmd,pkt.header.datalen);

	int iret = pNetObj->Send((char*)&pkt.header, pkt.header.datalen + PACKET_HEADER_SIZE);

	//LOG_DEBUG("2 Socket Send Msg To Client-cmd:%d--len:%d,iret:%d", pkt.header.cmd, pkt.header.datalen, iret);
