...
public:
		...
		, m_uiMaxLen(uiMaxFileLen), m_uiLen(0), m_uiDays(uiDays), m_uiLevel(LOG_LEVEL_DEBUG), m_bNeedTick(false)
	{
		...
		m_uiBackFileCount = 0;
		m_strShFileName = CHelper::GetExeDir();
	}
	...
	virtual bool Write(void const* pbuf, size_t nSize, bool bForce = false)
	{
		...
		if(m_iCurDay != pTime->tm_mday)
		{
			...
			// 新的一天创建新的目录保存日志文件
			std::stringstream oss_new_dir;
			oss_new_dir << m_strShFileName <<"logs_"<< pTime->tm_year + 1900
				<< "_" << std::setw(2) << std::setfill('0') << pTime->tm_mon + 1
				<< "_" << std::setw(2) << std::setfill('0') << pTime->tm_mday
				<< "/";

			if (0 != mkdir(oss_new_dir.str().c_str(), 0777))
			{
				//ERROR("mkdir %s error!!!\n",m_sdir.c_str());
			}

			std::string str_file = oss_new_dir.str() + oss.str();
			m_fileStream.open(str_file.c_str(), "ab+");
		...
		if(ret)
		{
			if(unlikely(m_uiLen >= m_uiMaxLen))
			{
				...
				*/
				...
				// 日志目录
				std::stringstream oss_dir;
				oss_dir << m_strShFileName << "logs_" << pTime->tm_year + 1900
					<< "_" << std::setw(2) << std::setfill('0') << pTime->tm_mon + 1
					<< "_" << std::setw(2) << std::setfill('0') << pTime->tm_mday
					<< "/";
				...
				std::stringstream oss_backname, oss_curname;
				// 1_log_back_0000_.2018-03-19.dat
				for (int i = 0; i <= MAX_BACK_FILE_COUNT; i++)
				{
					oss_backname.clear();
					oss_backname.str("");
					oss_backname << oss_dir.str() << m_oFilePre << "_back" << "_" << std::setw(4) << std::setfill('0') << m_uiBackFileCount
						<< "." << pTime->tm_year + 1900
						<< "-" << std::setw(2) << std::setfill('0') << pTime->tm_mon + 1
						<< "-" << std::setw(2) << std::setfill('0') << pTime->tm_mday
						<< ".dat";
					m_uiBackFileCount++;
					if (m_uiBackFileCount > MAX_BACK_FILE_COUNT)
					{
						m_uiBackFileCount = MAX_BACK_FILE_COUNT;
						break;
					}
					if (access(oss_backname.str().c_str(), F_OK) == 0)
					{
						// 文件存在 往上加1
						continue;
					}
				...
				// 1_log.2018-03-19.dat
				oss_curname << oss_dir.str() << m_oFilePre << "." << pTime->tm_year + 1900
					<< "-" << std::setw(2) << std::setfill('0') << pTime->tm_mon + 1
					<< "-" << std::setw(2) << std::setfill('0') << pTime->tm_mday
					<< ".dat";
...
private:
	void DelTimeOutLog(unsigned int uiDaysAgo)
	{
		...
		// 日志目录
		std::stringstream oss_old_dir;
		oss_old_dir << m_strShFileName << "logs_" << pAgoTime->tm_year + 1900
			<< "_" << std::setw(2) << std::setfill('0') << pAgoTime->tm_mon + 1
			<< "_" << std::setw(2) << std::setfill('0') << pAgoTime->tm_mday
			<< "/";

		//删除日志
		oss_agoname << oss_old_dir.str() << m_oFilePre << "." << pAgoTime->tm_year + 1900
			<< "-" << std::setw(2) << std::setfill('0') << pAgoTime->tm_mon + 1
			<< "-" << std::setw(2) << std::setfill('0') << pAgoTime->tm_mday
			<< ".dat";
		if (access(oss_agoname.str().c_str(), F_OK) == 0)
		{
			unlink(oss_agoname.str().c_str());
		}
		...
		// 1_log_back_0000_.2018-03-19.dat
		// 循环删除备份文件
		unsigned int uiBackFileCount = 0;
		for (int i = 0; i <= MAX_BACK_FILE_COUNT; i++)
		{
			oss_agobackname.clear();
			oss_agobackname.str("");
			oss_agobackname << oss_old_dir.str() << m_oFilePre << "_back" << "_" << std::setw(4) << std::setfill('0') << uiBackFileCount
				<< "." << pAgoTime->tm_year + 1900
				<< "-" << std::setw(2) << std::setfill('0') << pAgoTime->tm_mon + 1
				<< "-" << std::setw(2) << std::setfill('0') << pAgoTime->tm_mday
				<< ".dat";
			uiBackFileCount++;
			if (access(oss_agobackname.str().c_str(), F_OK) == 0)
			{
				// 文件存在 删除
				unlink(oss_agobackname.str().c_str());
			}
...
protected:
	...
	bool 		 m_bNeedTick ;
	unsigned int m_uiBackFileCount;
	std::string	 m_strShFileName;
};
